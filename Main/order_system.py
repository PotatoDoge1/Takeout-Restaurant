def place_order(menu):
    ...
    while ordering:
        ...
        # TODO: Extract the food category and the second_choice for each category
        for main_choice, second_choice in menu.items():

            # TODO: Loop through the second_choice for each food category
            for meal, price in second_choice.items():
            # TODO: Extract the meal and the price for each option
                ...
                print_menu_line(i, main_choice, meal, price)

                # Update the menu selection number
                i += 1

        # TODO: Ask customer to input menu item number
        choice = input("Please select your order's number: ")
        ...
        order = update_order(order, choice, menu_items)
        ...
        if order_more.upper() == "N":
            ...
            order_total = sum(price_list)
            order_total_rounded = round(order_total, 2)
    ...
    return order, order_total_rounded
...
def print_menu_line(index, main_choice, meal, price):
    """
    Prints a line of the menu.

    Parameters:
    index (int): The menu item number.
    main_choice (str): The category of the food item.
    meal (str): The name of the meal item.
    price (float): The price of the meal item.
    """
    # Print the menu item number, food category, meal, and price
    num_item_spaces = 32 - len(main_choice + meal) - 3
    item_spaces = " " * num_item_spaces
    if index < 10:
        i_spaces = " " * 6
    else:
        i_spaces = " " * 5
    print(f"{index}{i_spaces}| {main_choice} - {meal}{item_spaces} | ${price}")
...
def get_menu_items_dict(menu):
    ...
    for main_choice, second_choice in menu.items():
        # Loop through the second_choice for each food category
        for meal, price in second_choice.items():
            # Store the menu item number, item name and price in the menu_items
            menu_items[i] = {
                "Item name": main_choice + " - " + meal,
                "Price": price
            }
            i += 1
